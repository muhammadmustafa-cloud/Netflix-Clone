    ...
    <SideBar>
        ...
        <div className="w-full grid md:grid-cols-2 gap-6">
            ...
            <div className="flex flex-col gap-2">
                <p className="text-border font-semibold tex-sm">Image without Title</p>
                <Uploader/>
                <div className="w-32 h-32 p-2 bg-main border border-border rounded">
                    <img src="Images/99.jpg" alt="" className='w-full h-full object-cover rounded ' />
                </div>
            </div>
            {/* Image with title */}
            <div className="flex flex-col gap-2">
                <p className="text-border font-semibold tex-sm">Image with Title</p>
                <Uploader/>
                <div className="w-32 h-32 p-2 bg-main border border-border rounded">
                    <img src="Images/88.jpg" alt="" className='w-full h-full object-cover rounded ' />
                </div>
            </div>
        </div>
        {/* Description */}
            ...
            <div className="grid 2xl:grid-cols-4 lg:grid-cols-3 sm:grid-cols-4 grid:cols-2 gap-4 ">
                {
                    UsersData.map((user,i)=>(
                        <div key={i} className="p-2 italic text-xs text-text rounded flex-colo bg-main border border-border ">
                            <img src={`/Images/${user.image ? user.image : "user.png"}`} alt={user.fullName} className='w-full h-24 object-cover rounded mb-2'  />
                            <p>{user.fullName}</p>
      ...
      {/* Submit */}
         <button className="bg-subMain w-full flex-rows gap-6 font-medium text-white py-4 rounded"><ImUpload/>Publish Movie</button>
      </div> 
    </SideBar>
  )
}
